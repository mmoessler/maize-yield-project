# Acquire the FAOSTAT Government Expenditure dataset.
...
check_required_packages(c("here", "stringr"))

library(here)
library(stringr)

raw_zip <- here(
  "data", "source", "faostat-government-expenditure.csv.zip"
)
raw_csv <- here(
  "data", "source", "faostat-government-expenditure.csv"
)

# FAOSTAT bulk-download URLs can change. Confirm the current endpoint before a
# course refresh.
faostat_url <- paste0(
...
tryCatch(
  {
    message("Downloading FAOSTAT Government Expenditure data...")
    ...
    normalized_file <- extracted[
      str_detect(basename(extracted), "All_Data_\\(Normalized\\)\\.csv$")
    ][1]

    if (is.na(normalized_file)) {
      stop("Normalized Government Expenditure CSV not found in archive.")
    }
  ...
  error = function(e) {
    stop(
      "FAOSTAT Government Expenditure acquisition failed: ",
      conditionMessage(e), "\n",
...
message("Raw Government Expenditure input ready: ", raw_csv)
